...
public class ChatServerGUI extends Application implements ChatServerGuiInterface {
   ...
   * @param sendBufferSize Sendpuffergroeße, die der Server nutzen soll
   * @param receiveBufferSize Empfangspuffergroesse, die der Server nutzen soll
   */
  private void startChatServer(
      String implType, int serverPort, int sendBufferSize, int receiveBufferSize) throws Exception {
    ...
    try {
      chatServer =
          ServerFactory.getServer(serverImpl, serverPort, sendBufferSize, receiveBufferSize, this);
  ...
  /** GUI-Feld fuer eingeloggte CLients ueber Event-Liste des FavaFX-GUI-Threads aktualisieren */
    ...
    Platform.runLater(
        new Runnable() {
          ...
          public void run() {
            ...
            log.debug(
                "runLater: Logged in Clients: " + String.valueOf(loggedInClientCounter.get()));
  ...
  /**
   * GUI-Feld fuer Anzahl empfangener Requests ueber Event-Liste des FavaFX-GUI-Threads
   * aktualisieren
   */
